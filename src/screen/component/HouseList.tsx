import { Image, Pressable, StyleProp, StyleSheet, Text, View, ViewStyle, ViewToken } from 'react-native'
import React from 'react'
import Animated, { FadeInUp, useAnimatedStyle, withTiming } from 'react-native-reanimated';
import { Iconify } from 'react-native-iconify';
import { useTheme } from 'react-native-paper';
import { RootParamList, ViewableItem, windowDimension } from '../../GlobalTypes';
...
const HouseList: React.FC<RenderHouseProps> = React.memo(({ item, viewableItem, style }) => {
    ...
    const handlePress = (item: HouseItem) => {
        navigation.navigate("SingleHome", { item: item })
    }

    return (
        <>
            <Pressable className='flex-1' onPress={() => handlePress(item)}>
                <Animated.View className='flex-row rounded-14 p-2 space-x-1' style={[{ backgroundColor: theme.colors.onPrimary, }, rStyle, style]}>
                    {/* Image */}
                    <View>
                        <Animated.Image sharedTransitionTag={item.houseName} style={{ width: windowDimension.width / 3.6, height: windowDimension.width / 3.9, }}  // Set explicit width and height
